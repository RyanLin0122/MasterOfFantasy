    ...
    public Player CreatePlayerData(CreateInfo info)
    {
        if (!CharacterNames.Contains(info.name))
        {
            Player player = new Player
            {
                ...
                MiniGameRatio = 1,
                TeamID = -1,
                diaryInformation = new DiaryInformation(),
                Skills = new Dictionary<int, SkillData>(),
                EasySuccess = new int[3],
                EasyFail = new int[3],
                NormalSuccess = new int[3],
                NormalFail = new int[3],
                HardSuccess = new int[3],
                HardFail = new int[3],
                TitleCollection = new List<int>(),
                HighestMiniGameScores = new int[8],
                TotalMiniGameScores = new int[8],
                AcceptableQuests = new List<NQuest>(),
                FinishedQuests = new List<NQuest>(),
                ProcessingQuests = new List<NQuest>(),
                BadgeCollection = new List<int>(),
                CurrentBadge = -1,
                Cart = new List<CartItem>(),
                Honor = 0,
                Hotkeys = new List<HotkeyData>(),
                MonsterKillHistory = new Dictionary<int, int>(),
                CriticalBar = 0,
                Manufactures = new List<int>(),
                BattleWinTimes = 0,
                BattleLoseTimes = 0,
                PVPRank = 0,
                PVPLoseTime = 0,
