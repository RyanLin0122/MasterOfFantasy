...
namespace PolyNav
{
    ...
    public class PolyNav2D : MonoBehaviour
    {
        ...
        void TryNextFindPath()
        {
            ...
            {
                currentRequest.start = GetCloserEdgePoint(currentRequest.start, transform.position);
            }
        ...
        public Vector2 GetCloserEdgePoint(Vector2 point, Vector2 agent)
        {
            ...
            if (closerVertex == Vector2.zero)
            {
                Vector2 EdgePoint;
                print("Invoke Calculate EdgePoint2");
                CalculateEdgePoint2(agent, point, out EdgePoint);
                closerVertex = EdgePoint;
            }
            possiblePoints.Add(closerVertex);
            //possiblePoints = possiblePoints.OrderBy(vector => (point - vector).sqrMagnitude).ToArray(); //Not supported in iOS?
            //return possiblePoints[0];
        ...
        public void CalculateEdgePoint(Vector2 Agent, Vector2 Des, out Vector2 CloseEdgePoint)
        {
            List<(float, Vector2)> AllPoints = new List<(float, Vector2)>();
            foreach (var poly in map.allPolygons)
            {
                foreach (var point in poly.points)
                {
                    float dist = (Des - point).magnitude;
                    print(AllPoints.Count);
                    AllPoints.Add((dist, point));
                }
            }
            QuickSort(AllPoints, 0, AllPoints.Count - 1);
            Vector2 V1 = AllPoints[0].Item2;
            Vector2 V2 = AllPoints[1].Item2;
            float Slope1 = (Des.y - Agent.y) / (Des.x - Agent.x);
            float Slope2 = (V2.y - V1.y) / (V2.x - V1.x);
            float Edge_X = (V2.y - Des.y + Des.x * Slope1 - V2.y * Slope2) / (Slope1 - Slope2);
            float Edge_Y = Slope1 * Edge_X + (Des.y - Des.x * Slope1);
            CloseEdgePoint = new Vector2(Edge_X, Edge_Y);
            CloseEdgePoint = CloseEdgePoint - (CloseEdgePoint - Agent).normalized * 0.01f;
        }
        public void CalculateEdgePoint2(Vector2 Agent, Vector2 Des, out Vector2 CloseEdgePoint)
        {
            Vector2 NewDes = Agent + (Des - Agent) * 0.9f;
            if (PointIsValid(NewDes))
            {
                CloseEdgePoint = NewDes;
                return;
            }
            else
            {
                CalculateEdgePoint2(Agent, NewDes, out CloseEdgePoint);
            }
        }

        public static void QuickSort(List<(float, Vector2)> list, int left, int right)
        {
            if (right <= left)
                return;
        ...
        private static void Swap(List<(float, Vector2)> list, int indexA, int indexB)
        {
            var tmp = list[indexA];
