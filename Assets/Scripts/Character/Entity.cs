...
public static class ExtendFunction
{
    ...
    {
        return self.Type == EntityType.Monster ? string.Format("({0}:pos:{1},dir:{2},spd:{3}", self.Id, self.Position.String(), self.Direction.String(), self.Speed) : string.Format("({0}:pos:{1},dir:{2},spd:{3}", self.EntityName, self.Position.String(), self.Direction.String(), self.Speed);
    }
    ...
    {
        return new Vector3Int((int)nVector.X, (int)nVector.Y, (int)nVector.Z);
    }
