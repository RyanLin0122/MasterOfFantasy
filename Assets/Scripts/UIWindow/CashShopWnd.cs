...
public class CashShopWnd : Inventory
{
    ...
    {
        AudioSvc.Instance.PlayUIAudio(Constants.SmallBtn);
        MessageBox.Show("確定要購買 " + InventoryManager.Instance.itemList[ResSvc.Instance.CashShopDic[cata][CurrentTag][Order].ItemID].Name + "嗎?", MessageBoxType.Confirm,
            () =>
            {
                new CashShopSender(
            1,
    ...
    public void TryOnOffEquipment(int ItemID)
    {
        ...
        if (EquipmentWnd.GetEquipmentByItemID(ItemID, equips) == null)
        {
            ...
            TryOnEquipment(InventoryManager.Instance.GetItemById(ItemID));
        }
        else
        {
            Item item = InventoryManager.Instance.GetItemById(ItemID);
